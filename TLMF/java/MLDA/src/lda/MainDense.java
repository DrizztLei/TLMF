...
public class MainDense
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		*/
		...
		savePhiMatrix(phiFileName, info, corpus.getVocabulary(), ldaGibbsSampler);
	...
	private static void savePhiMatrix(String phiFileName, String[][][] info, Vocabulary vocabulary, LdaGibbsSampler ldaGibbsSampler)
	{
		// TODO Auto-generated method stub
		...
		try
		{
			...
			for(int i = 0; i < nameList.length; i++)
			{
				...
				int wordID = vocabulary.getId(name);
				

				for(int j = 0; j < TYPE; j++)
				{
					word2topicMatrix[i][j] = (float)(ldaGibbsSampler.nw[wordID][j]) / (float)(ldaGibbsSampler.nwsum[j]);
				}
			}
